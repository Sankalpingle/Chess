...
public class ChessGame extends JFrame {
    ...
    // Styles
    private final Color lightColor = new Color(176, 196, 222); // Steel blue
    private final Color darkColor = new Color(70, 130, 180); // Steel blue (darker)
    private final Border highlightBorder = BorderFactory.createLineBorder(new Color(200, 200, 200), 3); // Light grey
    private final Border captureBorder = BorderFactory.createLineBorder(Color.RED, 3); // Red border for captures
    private final Border selectBorder = BorderFactory.createLineBorder(new Color(0x456882), 3);
    ...
    private void refreshBoardUI() {
        ...
        if (selR != null && selC != null) {
            ...
            for (Move m : currentLegalMoves) {
                ...
                boolean isCapture = board[m.toR][m.toC] != null || m.enPassant;
                if (isCapture) {
                    squares[m.toR][m.toC].setBackground(new Color(255, 182, 193)); // Light red
                    squares[m.toR][m.toC].setBorder(captureBorder); // Red border for captures
                } else {
                    squares[m.toR][m.toC].setBackground(new Color(0xCBDCEB)); // Custom color for regular moves
                    squares[m.toR][m.toC].setBorder(highlightBorder); // Light grey border for regular moves
                }
            }
        }
    ...
    private void makeMove(Move m) {
        ...
        if (m.enPassant && moving.type == PieceType.PAWN) {
            ...
            // Pawn promotion - show dialog to select piece
            PieceType promotionPiece = showPromotionDialog(moving.color);
            board[m.toR][m.toC] = new Piece(promotionPiece, moving.color);
    ...
    private void addPawnMoveWithPromotion(List<Move> out, int fromR, int fromC, int toR, int toC, ColorSide side) {
        if ((side == ColorSide.WHITE && toR == 0) || (side == ColorSide.BLACK && toR == 7)) {
            out.add(new Move(fromR, fromC, toR, toC, null)); // Will be handled in makeMove
        } else {
    ...
    private PieceType showPromotionDialog(ColorSide color) {
        String[] options = {"Queen", "Rook", "Bishop", "Knight"};
        String[] unicodeSymbols = {
            color == ColorSide.WHITE ? "♕" : "♛", // Queen
            color == ColorSide.WHITE ? "♖" : "♜", // Rook
            color == ColorSide.WHITE ? "♗" : "♝", // Bishop
            color == ColorSide.WHITE ? "♘" : "♞"  // Knight
        };
        
        String[] displayOptions = new String[4];
        for (int i = 0; i < 4; i++) {
            displayOptions[i] = unicodeSymbols[i] + " " + options[i];
        }
